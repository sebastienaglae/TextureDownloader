...
namespace TextureDownloader.Texture.Utils
{
    public abstract class Download
    {
        ...
        {
            double size = 0;
            foreach (TextureRessources texture in textures)
                if (int.TryParse(texture.Size, out int r))
                    size += r;
        ...
        {
            return await DownloadFile(GetDownloadUrl(textureWebsite), $"cache_manifest_{textureWebsite}", textureWebsite.ToString(), webClient);
        }
        ...
        private static string? GetDownloadUrl(TextureWebsite textureWebsite)
        {
            switch (textureWebsite)
            {
                case TextureWebsite.AMBIENT_CG:
                    return "https://ambientcg.com/api/v2/downloads_csv";
            }
